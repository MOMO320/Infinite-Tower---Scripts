...
public class TowerTargetingRE : MonoBehaviour
{
    ...
    [SerializeField] private float fireTimeMax = 2.0f;  // 발사 주기(최대)  // 5초 마다 쏘겠다.
    ...
    private void Start()
    {
        bulletMoveSpeed = 9.5f;
        tower_Hp = 25f;
        hpCanvas = this.gameObject.transform.parent.parent.Find("HpCanvas_Tower").gameObject;
        hpSlider = this.gameObject.transform.parent.parent.Find("HpCanvas_Tower").GetComponentInChildren<Slider>();
        towerBody = this.gameObject.transform.parent.parent.Find("TowerBody").gameObject;
        isOnce = false;
    ...
    private void Update()
    {
            ...
            if (target != null)
            {
              ...
              //  transform.LookAt(target.transform.position);
                ...
                if (fireTimeMin > fireTimeMax)
                {
                    ...
                    // 미사일 생성
                    aBolt = Instantiate(bullet, firePos.position, bullet.transform.rotation, transform);
                    bulletList.Add(aBolt);
                     var cannon = aBolt.GetComponent<Cannon>();
                    var rigidbody = aBolt.GetComponent<Rigidbody>();
                    Vector3 vecAddPos = (firePos.forward * bulletMoveSpeed);
                    cannon.m_target = target;                    // 타겟의 정보를 넣어준다.
                   
                }
                
                if(aBolt != null && aBolt.GetComponent<Cannon>().isConnect == false)
                {
       ...
       if(hpSlider.value <= 0)
       {
            ...
            this.transform.parent.Find("cannon_Body").gameObject.SetActive(false);
